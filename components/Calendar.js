  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="grid grid-cols-5 gap-4">
        ...
        <p
          className={
            "text-center col-span-3 capitalized whitespace-nowrap text-indigo-600 text-xl " +
            fugaz.className
          }
        >
              ...
              {dayList.map((dayOfWeek, dayOfWeekIndex) => {
                ...
                return (
                  <div
                    ...
                    className={
                      "text-xs sm:text-sm border-2 border-solid p-2 flex items-center gap-2 justify-between rounded-lg " +
                      (isToday ? " border-black" : " border-indigo-200") +
                      (color === "white" ? " text-indigo-500" : " text-white")
                    }
                    key={dayOfWeekIndex}
                  >
                    <p className="sm:text-lg font-bold">{dayIndex}</p>
                  </div>
                );
              })}
            </div>
